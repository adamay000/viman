...
export class App {
  ...
  public constructor() {
    ...
    // Handle absolute path generated by next
    if (!isDev) {
      const isWindows = process.platform === 'win32'
      protocol.interceptFileProtocol('file', (request, callback) => {
        // On windows it always starts with file:///C:/_next/static
        // We should remove /C: from request
        const path = new URL(decodeURIComponent(request.url)).pathname.substr(isWindows ? 3 : 0)

        // If request is not next static file, keep it as is
        if (!path.startsWith('/_next')) {
          callback(isWindows ? `C:${path}` : path)
          return
        }

        // If request is next static file, convert correct path
        callback(join(__dirname, '../', path))
      })
    }
